...
describe("单元测试：Chainlink VRF", async () => {
  async function deployVRFConsumerFixture() {
    ...
    const VRFCoordinatorContract = await ethers.getContractFactory(
      "VRFCoordinatorV2Mock"
    );
    const VRFCoordinator = await VRFCoordinatorContract.deploy(
      BASE_FEE,
      GAS_PRICE_LINK
    );
    ...
    const VRFConsumer = await VRFConsumerContract.connect(deployer).deploy(
      subscriptionId,
      vrfCoordinatorAddr,
      keyHash
    );
  ...
  it("单元测试 7: VRF 请求成功发送", async function () {
    const { VRFConsumer, VRFCoordinator } = await deployVRFConsumerFixture();
  ...
  it("单元测试 8: 成功接受到随机数", async function () {
    ...
    await expect(
      VRFCoordinator.fulfillRandomWords(requestId, VRFConsumer.address)
    ...
    assert(
      rand0.gt(ethers.constants.Zero),
      "1st random number is not greater than 0"
    );
    assert(
      rand1.gt(ethers.constants.Zero),
      "2nd random number is not greater than 0"
    );
    assert(
      rand2.gt(ethers.constants.Zero),
      "3rd random number is not greater than 0"
    );
    assert(
      rand3.gt(ethers.constants.Zero),
      "4th random number is not greater than 0"
    );
    assert(
      rand4.gt(ethers.constants.Zero),
      "5th random number is not greater than 0"
    );
  });
